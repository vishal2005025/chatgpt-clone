...
const currentChat = useChatStore((state) =>
  state.chats.find((chat) => chat._id === currentChatId)
);
          ...
          {chats.map((chat) => (
            ...
            >
              ...
              <Button
                ...
                className="opacity-100 md:opacity-0 md:group-hover:opacity-100 h-8 w-8 active:bg-[#ebebeb] rounded-md cursor-pointer"
      ...
      <ScrollArea className="flex-1 px-2 overflow-y-auto min-h-0 custom-scrollbar">
        {isChatLoading ? (
          <div className="flex justify-center py-4">
            <p className="text-m text-muted-foreground" aria-live="polite">Loading chats...</p>
          </div>
        ) : (
          <>
            <SidebarSection
